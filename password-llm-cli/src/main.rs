...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(gpus) = gpus {
        ...
        gpu_index = Some(if gpu_names.len() > 1 {
            Select::with_theme(&ColorfulTheme::default())
                .with_prompt(format!(
                    "You have multiple ({}) GPUs in your system. Please select one.",
                    gpus.len()
                ))
                .default(0)
                .items(&gpu_names[..])
        ...
        let gpu = password_llm::gpu::obtain_stats_from_index(
            gpu_index.as_ref().unwrap().to_owned(),
            false,
        )
        .unwrap()
        .unwrap();
    ...
    let evaluation_result = password_llm::evaluate_password_strength(
        password,
        std::fs::File::open(ROCKYOU_PATH.join("rockyou.txt"))?,
        gpu_index,
    )
    .await?;
    println!(
    ...
    match evaluation_result.strength {
        PasswordStrength::ProneToBruteforceAttack => {
            println!("\tYour password is incredibly easy to bruteforce!")
        }
        PasswordStrength::ProneToDictionaryAttack => {
            println!("\tYour password is vulnerable to dictionary-based bruteforce attacks!")
        }
...
async fn download_rockyou() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut writer =
        std::io::BufWriter::new(std::fs::File::create(ROCKYOU_PATH.join("rockyou.txt"))?);
    let mut written = 0;
    ...
    while let Some(bytes) = stream.next().await {
        ...
        write!(
            lock,
            "Downloading rockyou.txt: {}/{} bytes\r",
            written, file_size
        )?;
    }
    writer.flush()?;
